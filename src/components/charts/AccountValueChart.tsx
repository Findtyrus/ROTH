    ...
    <div style={{ width: '100%', height: 320 }}>
      <ResponsiveContainer>
        <LineChart data={data} margin={{ top: 10, right: 20, left: 10, bottom: 0 }}>
          ...
          />
          <Line dataKey="traditional" name="Traditional IRA" stroke="var(--blue-light)" strokeWidth={2.5} dot={false} />
          <Line dataKey="rothIRA" name="Roth (Tax from IRA)" stroke="var(--orange-light)" strokeWidth={2.5} dot={false} strokeDasharray="6 3" />
          <Line dataKey="rothCash" name="Roth (Tax from Cash)" stroke="var(--green-light)" strokeWidth={2.5} dot={false} />
          {breakevenB && (
            <ReferenceLine x={breakevenB} stroke="var(--orange)" strokeDasharray="4 4" label={{ value: `BE-B: ${breakevenB}`, fill: 'var(--orange)', fontSize: 10, position: 'insideTopLeft' }} />
          )}
          {breakevenC && (
            <ReferenceLine x={breakevenC} stroke="var(--green)" strokeDasharray="4 4" label={{ value: `BE-C: ${breakevenC}`, fill: 'var(--green)', fontSize: 10, position: 'insideTopRight' }} />
          )}
        </LineChart>
      </ResponsiveContainer>
    </div>
